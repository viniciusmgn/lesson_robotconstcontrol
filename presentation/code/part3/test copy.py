...
def control_fun(_q, _robot, _htm_tg, _obstacles, _humans, _tray, hist_dist_human, _vel_human, _holding_tray, _care_obstacles, _old_ds):
    ...
    for obs in _obstacles:
        ...
        ds = robot.compute_dist(q = _q, obj = obs, h=0.05, eps=0.02, tol=1e-5,no_iter_max=2000, max_dist=0.6)
        ...
        if _holding_tray and _care_obstacles:
            for obs in _obstacles:
                point_disk, point_obs, dist, _ = _tray.list_of_objects[0].compute_dist(obs, h=0.05, eps=0.02, tol=1e-5,no_iter_max=2000)
    ...
    for j in range(len(_humans)):
        ds = _robot.compute_dist(q = _q, obj = _humans[j].list_of_objects[-1], h=0.05, eps=0.02, tol=1e-5,no_iter_max=2000, max_dist=3.0)
        ...
        if _holding_tray and _care_obstacles:
            point_tray, point_hmn, dist, _ = _tray.list_of_objects[0].compute_dist(_humans[j].list_of_objects[-1], h=0.05, eps=0.02, tol=1e-5,no_iter_max=2000)
