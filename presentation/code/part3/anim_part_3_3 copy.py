...
platform3 = ub.Box(htm=ub.Utils.trn([-1.2+0.3+0.1,-0.5+0.15,0.725]),width=0.6,depth=0.3,height=0.45,mesh_material=material_steel)
platform4 = ub.Box(htm=ub.Utils.trn([-1.2+0.3+0.1,0.5-0.15,0.725]),width=0.6,depth=0.3,height=0.45,mesh_material=material_steel)
platform5 = ub.Box(htm=ub.Utils.trn([-0.8+0.1,0,0.9+0.15]),width=0.8,depth=1.0,height=0.2,mesh_material=material_steel)
...
disk = ub.Cylinder(htm=ub.Utils.trn([0.3,0,0.5+0.02]),height=0.04,radius=0.05,mesh_material=material_gold)
...
def compute_control(_q, _htm_tg, is_handling_disk, care_obstacles):
    ...
    if care_obstacles:
        for obs in all_obstacles:
            ds = robot.compute_dist(q = _q, obj = obs, h = h, eps=eps)
    ...
    if is_handling_disk and care_obstacles:
        ...
        for obs in all_obstacles:
            point_disk, point_obs, dist, _ = disk.compute_dist(obs, h=h, eps=eps)
