...
model_drone = ub.Model3D(
    ...
    scale=2.0, mesh_material=ub.MeshMaterial.create_rough_metal())

drones= []
drone_colors = ['red','green','blue','yellow','magenta','cyan','gray','brown']
...
for i in range(param_n_robots):
    path_points.append(random_path_planner(start_points[i], all_tg_points[i], pc, param_radius, [-2., 2., -2. , 2., 0.5, 1.0]))
...
# def dofun(_q, _i, _pc):
#     qi = _q[3*_i:3*(_i+1),:]
#     return _pc.projection(qi)[1]-0.32
...
while cont:
    ...
    for j in range(param_n_robots):
        ...
        if finished[j]:
            print("Robot "+str(j+1)+" FINISHED  = "+str(round(error,2)))
        else:
            print("Robot "+str(j+1)+" index "+str(init_index[j])+" = "+str(round(error,2)))
            
        
        if error <=0.15:
            init_index[j]+=1
            if init_index[j] == len(path_points[j]):
                init_index[j] = len(path_points[j])-1
                finished[j] = True
