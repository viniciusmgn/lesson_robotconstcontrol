...
for step in range(5):
    ...
    ax.text(x_curve[-1]+0.3, y_curve[-1], r'$\mathcal{P}$', color='white', fontsize=25)
    ax.scatter(p[0], p[1], color=point_color, s=150, zorder=10)
    ax.text(p[0] + 0.1, p[1], r'$q$', color='white', fontsize=25)
    
    ax.scatter(proj[0]+2, proj[1], color='#191919', s=150, zorder=10)

    if step >= 1:
        ax.scatter(p[0], p[1], color=point_color, s=150)
        ax.text(proj[0]-0.7, proj[1]-0.1, r'$\Pi_{\mathcal{P}}(q)$', color='white', fontsize=25)
    ...
    if step >= 2:
        ...
        ax.text(( arrow_normal[0]+0.1) , (arrow_normal[1]-0.1) ,
                r'$N_{\mathcal{P}}(q)$', color='white', fontsize=25)
    ...
    if step >= 3:
        ...
        ax.text((arrow_tangent[0]+0.1) , (arrow_tangent[1]-0.1) ,
                r'$T_{\mathcal{P}}(q)$', color='white', fontsize=25)
    ...
    if step >= 4:
        ...
        ax.arrow(p[0], p[1], arrow_tangent[0] - p[0], arrow_tangent[1] - p[1],
                 color=tangent_color, head_width=0.1, length_includes_head=True)
        ax.text((arrow_tangent[0]+0.1) , (arrow_tangent[1]-0.1) ,
                r'$T_{\mathcal{P}}(p)$', color='white', fontsize=25)
    ...
    # Save as SVG
    fig.savefig(f"/home/vinicius/Desktop/Aulas/Robot Constrained Control/presentation/images/part3/image3_Slide{step + 1}.svg", format='svg',  bbox_inches=None, pad_inches=0)
    plt.close(fig)
